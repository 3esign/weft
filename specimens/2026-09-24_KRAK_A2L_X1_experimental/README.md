# KRAK_A2L_X1 — five ways to hold a shelf in the air

**Print `KRAK_A2L_X1.gcode.3mf` from this folder.**
...
So the gate's ranking at 24 mm, best first: `splay` (4) < `truss` (9) < `tiprail` (31) < `plain` (47) < `castle` (69).
...
`KRAK_A2L_X1_plan.svg` in this folder draws the ten sectors with their method names.
...
|---|---|
| machine | Bambu A2L |
| support gate, declared ceiling | **PASS** — 0 problems over 500,226 points / 143 layers |
| first-layer audit | **PASS** — 1 island, preview Z equals model Z |
| gate at the evidenced 16.2 mm | 352 findings, **0 outside the declared zones** |
| package re-gate | PASS |
| Z levels | 142 |
| max Z | 34.08 mm |
| thread path | 276 m |
| filament | 11,341.59 mm ≈ **34.37 g** |
| estimated time | 5 h 20 m 16 s (kinematic estimate, not the printer's own) |
