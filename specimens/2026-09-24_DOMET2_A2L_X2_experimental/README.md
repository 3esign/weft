# DOMET2_A2L_X2 — corrected reissue

**Print `DOMET2_A2L_X2.gcode.3mf` from this folder.**

Supersedes `2026-09-24_DOMET2_A2L_X1_experimental`, which must not be printed.
...
Nothing, on this machine. The first-layer audit passes on both
`DOMET2_A2L_X1.gcode.3mf` and `DOMET2_A2L_X2.gcode.3mf` — model first Z and preview
first Z both read 0.240, with an identical first-layer segment count.
This matches what was observed in the slicer: the Bambu first layer previewed
correctly.

X2 is reissued for this machine anyway, so that both machines are built from the
same toolchain revision and the pair stays comparable. The only change in the
emitted file is the explicit `WIPE_START` / `WIPE_END` region and the
`WEFT_FIRST_LAYER_V1` declaration in the header.
...
2. The header declares `; WEFT_FIRST_LAYER_V1 Z=0.240 H=0.240`,
...
| check | result |
|---|---|
| support gate, declared ceiling | **PASS** — 0 problems over 562,479 points / 254 layers |
| first-layer audit | **PASS** — 1 island, model Z = preview Z = 0.240 |
| second gate at the evidenced 16.2 mm | 946 findings, **0 outside the declared zones** |
| Bambu package re-gate | PASS (re-gated after packing) |
...
| max Z | 60.72 mm |
| thread path | 322.7 m |
| filament | 13,508.50 mm ≈ **40.94 g** |
| weld nodes | 12,107 |
| estimated time | 6 h 12 m 19 s (kinematic estimate, not the printer's own) |
